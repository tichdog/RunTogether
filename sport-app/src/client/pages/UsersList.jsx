...
export function UsersList({ onSelect, currentUserId }) {
  ...
  return (
    <div
      className="page users-list-page"
    ...
    >
      ...
      <Card style={{ padding: 0, overflow: 'hidden' }}>
        <div className="users-list-header" style={headerGrid}>
          {['№', 'Пользователь', 'Email', 'Роль', 'Рейтинг', 'Статус', 'Верификация', ''].map(
            (h) => (
              <div key={h}>{h}</div>
            )
          )}
        ...
        {filtered.map((user, i) => (
          <div
            ...
            style={rowGrid(i, filtered.length)}
          >
            <span
              className="user-cell user-cell-id"
              style={{ fontSize: 12, color: T.textHint, fontWeight: 700 }}
            >
              {user.id}
            </span>
            <div
              className="user-cell user-cell-user"
              style={{ display: 'flex', alignItems: 'center', gap: 9 }}
            >
              <Avatar initials={user.initials} src={user.avatarUrl} size={28} />
              <span className="user-name" style={{ fontSize: 13, fontWeight: 700, color: T.text }}>
                {user.name}
              </span>
            </div>
            <span
              className="user-cell user-cell-email"
              style={{ fontSize: 13, color: T.textMuted }}
            >
              {user.email || 'скрыто'}
            </span>
            <div className="user-cell user-cell-role">
              <Badge text={ROLE_LABELS[user.role]} colors={ROLE_COLORS[user.role]} />
            </div>
            <span
              className="user-cell user-cell-rating"
              style={{ fontSize: 13, color: T.warning, fontWeight: 800 }}
            >
            ...
            <span
              className="user-cell user-cell-verified"
              style={{
                fontSize: 12,
                color: user.phoneVerified ? T.success : T.textHint,
                fontWeight: 600,
              }}
            >
...
const headerGrid = {
  display: 'grid',
  gridTemplateColumns:
    '56px minmax(150px, 1.1fr) minmax(160px, 1.35fr) minmax(126px, 0.75fr) 86px minmax(132px, 0.75fr) minmax(160px, 0.9fr) 90px',
  gap: 10,
  padding: '10px 18px',
  fontSize: 11.5,
  color: T.textHint,
