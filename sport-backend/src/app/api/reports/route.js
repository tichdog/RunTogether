...
export const POST = route(async request => {
  ...
  if (Number(reportedUserId) === Number(user.id)) throw badRequest("Нельзя отправить жалобу на себя");
  ...
  const report = await transaction(async client => {
    ...
    if (!isAdmin(target) && Number(countRows[0].count) >= Number(settings.auto_block_complaints_count)) {
      await client.query(
        `update users
