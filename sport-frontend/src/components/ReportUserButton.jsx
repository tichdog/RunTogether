...
export function ReportUserButton({ user, currentUserId, label = "Жалоба", compact = false, onCreated }) {
    ...
    <>
      ...
      {open && (
        ...
        >
          ...
          >
            <h2 style={{ margin: "0 0 4px", fontSize: 20, color: T.text }}>Жалоба на пользователя</h2>
            <p style={{ margin: "0 0 16px", color: T.textMuted, fontSize: 13 }}>{user.name}</p>

            <label style={fieldStyle}>
              <span style={labelStyle}>Причина</span>
              <Select value={reason} onChange={event => setReason(event.target.value)} style={{ width: "100%" }}>
            ...
            {message && (
              <div style={{ marginBottom: 12, color: message.includes("отправлена") ? T.success : T.danger, fontSize: 13 }}>
                {message}
              </div>
            )}

            <div style={{ display: "flex", justifyContent: "flex-end", gap: 8 }}>
              <Btn onClick={() => setOpen(false)} disabled={saving}>Отмена</Btn>
