...
export function Avatar({ color = "#2563EB", initials = "U", size = 32, src }) {
  if (src) {
    return <img alt="" src={src} style={{ width: size, height: size, borderRadius: "50%", objectFit: "cover" }} />;
  }
  return (
    <div style={{
      ...
      flexShrink: 0, color, fontSize: size * 0.35, fontWeight: 700,
    }}>
      {initials}
    </div>
  );
...
export function Badge({ text, colors }) {
  ...
  return (
    <span style={{
      ...
      borderRadius: T.radiusSm, padding: "3px 9px", fontSize: 12,
...
export function Btn({ children, onClick, variant = "default", danger, type = "button", disabled, style: extra }) {
  const variants = {
    default: { background: T.surface, color: T.text, border: `1px solid ${T.border}`, padding: "8px 14px" },
    primary: { background: T.accent, color: "#fff", border: `1px solid ${T.accent}`, padding: "8px 16px" },
    ghost: { background: "transparent", color: T.textMuted, border: "1px solid transparent", padding: "8px 14px" },
    danger: { background: T.dangerLight, color: T.danger, border: "1px solid #FECACA", padding: "8px 14px" },
  };
  ...
  return (
    <button
      ...
      disabled={disabled}
      style={{
        borderRadius: T.radiusSm, cursor: disabled ? "default" : "pointer",
        fontSize: 13, fontWeight: 600, display: "inline-flex", alignItems: "center",
        justifyContent: "center", gap: 6, fontFamily: "inherit", opacity: disabled ? 0.55 : 1,
        ...v, ...extra,
      }}
    >
      {children}
...
export function Card({ children, style: extra }) {
  return (
    <div style={{
      ...
      border: `1px solid ${T.border}`, padding: "20px 24px", ...extra,
    }}>
      {children}
    </div>
  );
...
export function SectionTitle({ children }) {
  return (
    <div style={{
      fontSize: 12, fontWeight: 700, color: T.textHint,
...
export function EmptyState({ children }) {
  return <div style={{ padding: 24, color: T.textMuted, fontSize: 13, textAlign: "center" }}>{children}</div>;
}
