...
import { Badge, Btn, Card, EmptyState, Input, Select, SectionTitle, StatusBadge } from "../components/ui";
import { T } from "../tokens";

const REPORT_STATUS = {
  open: { text: "Открыта", colors: { bg: T.warningLight, text: T.warning, border: "#FDE68A" } },
  warned: { text: "Предупреждение", colors: { bg: T.accentLight, text: T.accent, border: "#BFDBFE" } },
...
export function Reports({ onSelectUser }) {
  ...
  const moderate = async (report) => {
    ...
    try {
      ...
      setReports(prev => prev.map(item => Number(item.id) === Number(report.id) ? data.report : item));
  ...
  return (
    <div style={{ padding: "32px 36px", flex: 1, overflowY: "auto", minWidth: 0 }}>
      ...
      <div style={{ display: "inline-flex", gap: 4, border: `1px solid ${T.border}`, borderRadius: T.radiusSm, padding: 4, background: T.surface, marginBottom: 16 }}>
        <button type="button" onClick={() => { setView("open"); setFilter(""); }} style={tabStyle(view === "open")}>
          На проверке
        </button>
        <button type="button" onClick={() => { setView("archive"); setFilter(""); }} style={tabStyle(view === "archive")}>
          Архив
        </button>
      </div>

      {message && <div style={{ color: message.includes("сохранено") ? T.success : T.danger, marginBottom: 16, fontSize: 13 }}>{message}</div>}
      ...
      {!loading && (
        <div style={{ display: "grid", gap: 14 }}>
          ...
          {!filteredReports.length && (
            <EmptyState>{view === "open" ? "Открытых жалоб нет." : "В архиве пока нет жалоб."}</EmptyState>
          )}
        </div>
      )}
    </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "minmax(0, 1fr) auto", gap: 16, alignItems: "start" }}>
        <div>
          <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap", marginBottom: 10 }}>
            ...
            <span style={{ color: T.textHint, fontSize: 12 }}>#{report.id} · {new Date(report.createdAt).toLocaleString()}</span>
            {report.resolvedAt && (
              <span style={{ color: T.textHint, fontSize: 12 }}>закрыта {new Date(report.resolvedAt).toLocaleString()}</span>
            )}
          </div>
          <h2 style={{ margin: "0 0 8px", color: T.text, fontSize: 18 }}>{report.reason}</h2>
          {report.details && <p style={{ margin: "0 0 14px", color: T.textMuted, fontSize: 13, lineHeight: 1.5 }}>{report.details}</p>}
          <div style={{ display: "grid", gridTemplateColumns: "repeat(2, minmax(0, 1fr))", gap: 12 }}>
            <PersonBlock title="Кто пожаловался" person={report.reporter} onSelectUser={onSelectUser} />
            <PersonBlock title="На кого жалоба" person={report.reportedUser} onSelectUser={onSelectUser} target />
          </div>
          {!isOpen && (
            <div style={{ marginTop: 14, color: T.textMuted, fontSize: 13 }}>
              Решение: {status.text}{report.moderator?.name ? ` · ${report.moderator.name}` : ""}
        ...
        {isOpen && (
          <div style={{ width: 300, display: "grid", gap: 10 }}>
            ...
            <Select value={decision.action} onChange={event => onDecision({ action: event.target.value })} style={{ width: "100%" }}>
            ...
            {decision.action === "ban" && (
              <div style={{ display: "grid", gridTemplateColumns: "1fr 92px", gap: 8 }}>
                <Select value={decision.banMode} onChange={event => onDecision({ banMode: event.target.value })}>
...
function PersonBlock({ title, person, onSelectUser, target }) {
  return (
    <div style={{ border: `1px solid ${T.border}`, borderRadius: T.radiusSm, padding: 12, minWidth: 0 }}>
      <div style={{ color: T.textHint, fontSize: 11, fontWeight: 800, textTransform: "uppercase", marginBottom: 6 }}>{title}</div>
      <button
        onClick={() => onSelectUser?.({ id: person.id, name: person.name })}
        style={{ border: "none", background: "transparent", padding: 0, color: T.text, cursor: "pointer", fontSize: 14, fontWeight: 800 }}
      >
      ...
      {target && (
        <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap", marginTop: 8 }}>
          ...
          <span style={{ color: T.textMuted, fontSize: 12 }}>Предупреждений: {person.warnings || 0}</span>
        </div>
      )}
    </div>
  );
