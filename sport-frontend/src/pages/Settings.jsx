    ...
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "14px 0", borderBottom: `1px solid ${T.border}` }}>
      ...
      <button
        ...
        style={{
          ...
          background: value ? T.accent : T.border, cursor: "pointer", position: "relative",
        }}
      >
        <span style={{
...
export function Settings() {
  ...
  useEffect(() => {
    api.settings().then(data => setSettings(data.settings)).catch(err => setMessage(err.message));
  }, []);
  ...
  return (
    <div style={{ padding: "32px 36px", flex: 1, overflowY: "auto", minWidth: 0 }}>
      <div style={{ marginBottom: 28 }}>
        ...
        <p style={{ margin: "4px 0 0", fontSize: 13, color: T.textMuted }}>Конфигурация системы из PostgreSQL</p>
      </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 18, maxWidth: 760 }}>
        ...
        {message && <div style={{ color: message.includes("сохран") ? T.success : T.danger, fontSize: 13 }}>{message}</div>}
        <Btn variant="primary" onClick={save} style={{ width: 180 }}>Сохранить</Btn>
      </div>
    </div>
  );
}
...
const sectionHeader = {
  ...
  marginBottom: 4, paddingBottom: 12, borderBottom: `1px solid ${T.border}`,
};
