  ...
  const stats = useMemo(
    () => [
      { label: 'Пользователей', val: users.length, sub: 'зарегистрировано' },
      {
        label: 'Тренировок',
        val: workouts.length,
        sub: `${workouts.filter((w) => w.status === 'open').length} с открытым набором`,
      },
    ...
    <div style={{ padding: '32px 36px', flex: 1, overflowY: 'auto', minWidth: 0 }}>
      ...
      >
        {stats.map((s, i) => (
          <Card
            key={s.label}
            style={{ padding: '18px 20px', borderTop: `3px solid ${STAT_ACCENTS[i]}` }}
          >
      ...
      <div className="overview-content-grid">
        <Card>
          ...
          >
            ...
            <Btn onClick={() => setActive('Пользователи')} style={{ fontSize: 12 }}>
              Все пользователи
            </Btn>
          </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          ...
          {!activities.length && (
            <Card style={{ padding: 12, color: T.textMuted, fontSize: 13 }}>
              Активности пока нет.
            </Card>
          )}
        </div>
...
function activityTitle(item) {
  if (item.type === 'workout_created')
    return `Создана тренировка: ${item.workout_title || item.metadata?.title || ''}`
  if (item.type === 'achievement_earned')
    return `Получено достижение: ${item.metadata?.title || ''}`
  return item.type
}
